...
pageComp = new PageComponent #Need a nested pageComponent for section scroll
	...
	y: headerBar.y + headerBar.height + input_tab.height
	width: Screen.width
	height: IntelliStream_App.height - headerBar.height - input_tab.height - footerBar.height
	originY: 0
...
createScrollView = (num, parent) ->
	for i in [0...num]
		...
		pageCompSection.onClick ->
			...
			if current+3 > scrollBarDescriptions.length
				pageComp.opacity = 0
				WLP_Output.bringToFront()
				WLP_Output.opacity = 1
				output_tab.opacity = 1
