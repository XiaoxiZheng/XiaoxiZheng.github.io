...
wellName = ["Bishop ESP", "Green A 3-6 Unit", "Adam's Route", "Edison Network", "Mohal 5-Spot", "Green A3-6 Unit", "Frank's Route", "Sam's Network 1", "Goldwins Route", "Crozier Group #1", "Crozier A Route #2", "Crozier A Route #3", "Pohler Group A #2", "Rayes Unit BA #1", "Rayes Unit BA #2", "Writte S Route #1", "Writte S Route #2", "Writte S Route #3", "Writte S Route #4", "Writte S Route #5", "Writte S Route #6", "Writte S Route #7"]
...
updateBgColor = (bgColor_) ->
	...
	if darkMode
		...
		#Analysis Chart bg
		groupProductionChartContainer.image = "images/curves/barChartBg_dark.svg"
		analysisSave.image = "images/analysis-save_dark.png"
	else
		facetBg.opacity = 1
		n_facetBG.opacity = 1
		...
		pages[dashboardPage].content.children[0].image = "images/page_placeholders/D_Dashboard.png"
		#Analysis Chart bg
		groupProductionChartContainer.image = "images/curves/barChartBg.svg"
		analysisSave.image = "images/analysis-save.png"
...
# 		createDataTableHighChart(defaultFontColor,"tsDivId_" + i)
	...
	for i in [0...dataVizContainer.descendants.length]
		dataVizContainer.descendants[i].color = defaultFontColor_
...
topWellsContainer = new Layer
	...
	width: dashboardWidth/3, height: 260
	backgroundColor: bgColor
	borderWidth: 1
	borderColor: highlightColor
...
wellNameHeader = new Layer
	...
	backgroundColor: null
	parent: tableHeaderContainer
	html: '<div class="columnHeader">Group Name</div>'	
...
liftTypeHeader = new Layer
	...
	backgroundColor: null
	parent: tableHeaderContainer
	html: '<div class="columnHeader">Optimization</div>'
...
drawDashboardTable = () ->
	...
	# 		y: tableData.height
		...
		tableData.liftTypeEntry = new Layer
			width: columnWidth, height: 50, x: liftTypeHeader.x + 20, y: Align.center
			parent: tableData
			color: defaultFontColor
			backgroundColor: null
			html: '<div class="rowEntry">' + Utils.randomChoice(['Yes', 'No']) + '</div>'
		...
		tableData.oilEntry = new Layer
			...
			color: defaultFontColor
			backgroundColor: null
			html: '<div class="rowEntry">' + Utils.randomChoice([1000..10000]) + '</div>'
		...
		tableData.waterEntry = new Layer
			...
			color: defaultFontColor
			backgroundColor: null
			html: '<div class="rowEntry">' + Utils.randomChoice([100..10000]) + '</div>'
		...
		tableData.gasEntry = new Layer
			...
			color: defaultFontColor
			backgroundColor: null
			html: '<div class="rowEntry">' + Utils.randomChoice([1000..100000]) + '</div>'
			...
			for j in [2...rowContent.length]
				rowContent[j].animate
					...
					options:
						curve: easeOutQuart
						time: 0.7
	else #curves are not defined when animating 'out'. default: Bezier.easeInOut
		notification_facetFrame.animate
				opacity: 0
				options: 
					time: 0.2
...
chartHeaderContainer = new Layer
	parent: dataVizContainer
	width: 305
	height: 40
	x: 40
	backgroundColor: null 
	
chartHeader = new TextLayer
	parent: chartHeaderContainer
	fontFamily: "GE Inspira Sans"
	fontSize: 15
	color: defaultFontColor
	text: "BISHOP ESP GROUP"
chartDataSetToggle = new TextLayer
	parent: chartHeader
	fontFamily: "GE Inspira Sans"
	fontSize: 15
	y: chartHeader.height + 2
	color: defaultFontColor
	text: "GROUP OIL PRODUCTION"
	width: 200
toggleIcon = new FontAwesome
	parent: chartHeader
	icon: 'caret-down'
	color: defaultFontColor
	fontSize: 15
	y: chartDataSetToggle.y
	x: chartDataSetToggle.width + 5

currentOilLegendContainer = new Layer
	parent: dataVizContainer
	x: 40
	y: chartHeaderContainer.y + chartHeaderContainer.height + 35
	width: 140
	height: 50
	backgroundColor: null

currentOilLegend = new Layer
	parent: currentOilLegendContainer
	width: 8
	height: 14
	backgroundColor: oil_green
currentOilText = new TextLayer
	parent: currentOilLegendContainer
	width: 100
	height: 15
	x: currentOilLegend.x + currentOilLegend.width + 10
	fontFamily: "GE Inspira Sans"
	fontSize: 12
	color: defaultFontColor
	text: "Current Oil"
currentOilValue = new TextLayer
	parent: currentOilLegendContainer
	width: 100
	height: 35
	x: currentOilText.x
	y: currentOilText.y + currentOilText.height + 4 
	color: defaultFontColor
	fontFamily: "GE Inspira Sans"
	fontSize: 30
	text: "23414 std/d"
	
potentialOilLegendContainer = new Layer
	parent: dataVizContainer
	x: currentOilLegendContainer.x + currentOilLegendContainer.width + 50
	y: chartHeaderContainer.y + chartHeaderContainer.height + 35
	width: 140
	height: 50
	backgroundColor: null

potentialOilLegend = new Layer
	parent: potentialOilLegendContainer
	width: 8
	height: 14
	backgroundColor: oil_green
	opacity: 0.4
potentialOilText = new TextLayer
	parent: potentialOilLegendContainer
	width: 100
	height: 15
	x: potentialOilLegend.x + potentialOilLegend.width + 10
	fontFamily: "GE Inspira Sans"
	fontSize: 12
	color: defaultFontColor
	text: "Potential Oil"
potentialOilValue = new TextLayer
	parent: potentialOilLegendContainer
	width: 100
	height: 35
	x: potentialOilText.x
	y: potentialOilText.y + potentialOilText.height + 4 
	color: defaultFontColor
	fontFamily: "GE Inspira Sans"
	fontSize: 30
	text: "28414 std/d"
...
groupProductionChartContainer = new Layer
	parent: dataVizContainer
	width: 1110, height: 355, y: currentOilLegendContainer.y + currentOilLegendContainer.height + 20, x: 50
	backgroundColor: null
	image: "images/curves/barChartBg.svg"
...
drawDataViz = () ->
	dataViz = Highcharts.chart('groupProductionChartContainer',
		credits: {enabled: false}
		exporting: {enabled: false}
		chart: {type: 'column', renderTo: '',marginLeft: 50, marginRight: 20, marginTop: 35, marginBottom: 50, backgroundColor: null}
		title: null 
...
# 			title: {enabled: true, text: 'Showing 10/10 wells in this group', style: {color: defaultFontColor,fontSize: '12px'}}
			categories: ['Clark A #6', 'Bishop #14','Bishop #10','Clark A #5', 'Clark A #4', 'Bishop #11','Clark A #1','Clark A #9', 'Bishop #13', 'Bishop #12', 'Clark A #7']
# 			crosshair: true
		yAxis:
			visible: false
# 			min: 0
...
# 			tickLength: 0
# 			tickColor: null
# 			title: {enabled: false}# text: 'Production (stb/d)', style: {color: defaultFontColor, fontSize: '12px'}}
# 			labels: {enabled: false}#, style:{color: defaultFontColor, fontSize: '12px'}}
# 			tickInterval: null
...
# 			pointFormat: '<span class="row"><span class="alignLeft"><span style="color:{point.color}">\u25CF</span> {series.name} </span><span class="alignRight"><b> {point.y} (rb/d)</b></span></span>'
		...
		series: [
			{
				name: 'Current Oil',
				data: [800, 300, 750, 400, 500, 300, 550, 620, 620, 700, 700],
				color: '#7ED321',
				maxPointWidth: 26
			}, {
				name: 'Potential Oil',
				data: [820, 700, 850, 700, 550, 700, 680, 800, 650, 790, 850],
				color: '#7ED32166',
				maxPointWidth: 40
				},
			]
...
# 				}
	...
	groupProductionChartContainer.onMouseOver ->
	groupProductionChartContainer.onMouseOut ->
# )

sliderContainer = new Layer
	parent: analysisContainer
	x: 110
	y: groupProductionChartContainer.y + groupProductionChartContainer.height + 30
	backgroundColor: null
	width: 630 
	height: 150
	opacity: 0 
...
oilProductionSlider = new RangeSliderComponent
...
oilProductionSlider.fill.backgroundColor = oil_green
oilProductionSlider.borderColor = "#09819C"
oilProductionSlider.minKnob.width = 2
oilProductionSlider.minKnob.height = 20
oilProductionSlider.maxKnob.backgroundColor = blueGrey


oilProductionLabel = new TextLayer
	parent: oilProductionSlider
...
oilProductionValue1 = new TextLayer
	parent: oilProductionSlider
	x: oilProductionSlider.width + 15, y: -12, width: 50
	text: 2500
	borderColor: highlightColor
	borderWidth: 1
	padding: 8
...
oilProductionValue2 = new TextLayer
	parent: oilProductionSlider
	x: -60, y: -12, width: 50
	borderColor: highlightColor
	borderWidth: 1
...
oilProductionUnit = new TextLayer
	parent: oilProductionSlider
	x: oilProductionValue1.x + oilProductionValue1.width + 5, y: -5, width: 50
	text: "stb/d"
	fontSize: 12
	color: defaultFontColor
	fontFamily: "GE Inspira Sans"

#             WATER PRODUCTION SLIDER             #	
waterProductionSlider = new RangeSliderComponent
	parent: sliderContainer
	x: oilProductionSlider.width + 210, y: 180
	width: 210, height: 5
	min: 0, max: 4000
	minValue: 700, maxValue: 700
	knobSize: 20
waterProductionSlider.fill.backgroundColor = water_blue
waterProductionSlider.backgroundColor = "#B6C3CC"
# waterProductionSlider.knob.color = defaultFontColor
waterProductionSlider.minKnob.width = 2
waterProductionSlider.minKnob.height = 20
waterProductionSlider.maxKnob.backgroundColor = blueGrey

waterProductionLabel = new TextLayer
	parent: waterProductionSlider
	x: -60, y: -40, width: 200, height: 30
...
waterProductionValue1 = new TextLayer
	parent: waterProductionSlider
	x: waterProductionSlider.width + 15, y: -12, width: 50
	text: 2500
	fontSize: 12
	borderColor: highlightColor
	borderWidth: 1
...
waterProductionValue2 = new TextLayer
	parent: waterProductionSlider
	x: -60, y: -12, width: 50
	padding: 8
	text: 100
...
waterProductionUnit = new TextLayer
	parent: waterProductionSlider
	x: waterProductionValue1.x + waterProductionValue1.width + 5, y: -5, width: 50
	text: "stb/d"
	fontSize: 12
	color: defaultFontColor
	fontFamily: "GE Inspira Sans"
...
powerConsumptionSlider = new RangeSliderComponent
	parent: sliderContainer
	x: oilProductionSlider.width + waterProductionSlider.width + 410
	y: 180
	width: 210, height: 5
	min: 0, max: 400
	minValue: 80, maxValue: 80
	knobSize: 20
powerConsumptionSlider.fill.backgroundColor = power_orange
powerConsumptionSlider.backgroundColor = "#B6C3CC"
powerConsumptionSlider.borderRadius = 0
# powerConsumptionSlider.knob.color = "white"
powerConsumptionSlider.minKnob.width = 2
powerConsumptionSlider.maxKnob.height = 20
powerConsumptionSlider.maxKnob.backgroundColor = blueGrey

powerConsumptionLabel = new TextLayer
	parent: powerConsumptionSlider
	x: -60, y: -40, width: 200, height: 30
...
powerConsumptionValue1 = new TextLayer
	parent: powerConsumptionSlider
	x: powerConsumptionSlider.width + 15, y: -12, width: 50
	text: 725
	fontSize: 12
	borderColor: highlightColor
	borderWidth: 1
...
powerConsumptionValue2 = new TextLayer
	parent: powerConsumptionSlider
	x: -60, y: -12, width: 50
	padding: 8
	text: 100
...
powerConsumptionUnit = new TextLayer
	parent: powerConsumptionSlider
	x: powerConsumptionValue1.x + powerConsumptionValue1.width + 5, y: -5, width: 50
	text: "Kwh"
	fontSize: 12
	color: defaultFontColor
	fontFamily: "GE Inspira Sans"
...
animateSliders = () ->
	oilProductionSlider.animateToMaxValue(1800, {curve: easeOutQuart, time: 0.3})
	waterProductionSlider.animateToMaxValue(1900, {curve: easeOutQuart, time: 0.3})
	powerConsumptionSlider.animateToMaxValue(352, {curve: easeOutQuart, time: 0.3})
	
# Listen for slider value updates
oilProductionSlider.maxKnob.onDrag ->
# oilProductionSlider.onValueChange ->
	oilProductionValue1.text = Math.round(oilProductionSlider.maxValue)
	oilProductionValue2.text = Math.round(oilProductionSlider.maxValue/1.2)
	waterProductionValue1.text = Math.round(waterProductionSlider.maxValue)
	waterProductionValue2.text = Math.round(waterProductionSlider.maxValue/1.2)
	powerConsumptionValue1.text = Math.round(powerConsumptionSlider.maxValue)
	powerConsumptionValue2.text = Math.round(powerConsumptionSlider.maxValue/1.2)

	waterProductionSlider.animate
		value: oilProductionSlider.value / 1
		options: {curve: easeOutQuart, time: 0.3}
	powerConsumptionSlider.animate
		value: oilProductionSlider.value / 1.5
		options: {curve: easeOutQuart, time: 0.3}
	
oilProductionSlider.maxKnob.onDragEnd ->
	drawDataViz()

analysisSave = new Layer
	parent: analysisContainer
	width: 1165, height: 30, x: 50
	y: groupProductionChartContainer.height + oilProductionSlider.height + 400
	...
	if display
		...
		Utils.delay 0.2, ->
			...
			sliderContainer.animate
				opacity: 1
				y: groupProductionChartContainer.y + groupProductionChartContainer.height - 20
				options:
					delay: 0.3
					curve: easeOutQuart
					time: 0.7
...
# 				time: 0.2
		...
		oilProductionSlider.animateToMaxValue(oilProductionSlider.minValue, {curve: easeOutQuart, time: 0.1})
		waterProductionSlider.animateToMaxValue(waterProductionSlider.minValue,{curve: easeOutQuart, time: 0.1})
		powerConsumptionSlider.animateToMaxValue(powerConsumptionSlider.minValue,{curve: easeOutQuart, time: 0.1})
